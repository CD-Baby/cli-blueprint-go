...
| `testdata/golden/*.txt` | pinned help text; `make update-golden` rewrites it |
| `LICENSE` | proprietary, Virgin Music, with a grant for company work |
...
This repository is public for reference. It is not open source. Virgin Music
and CD Baby staff may use it for company work. Projects scaffolded by
`init.sh` carry the same `LICENSE`.
